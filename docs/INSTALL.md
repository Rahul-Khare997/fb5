...
Gets you the `/fb5` command and both judge agents (`verifier`, `taste-judge`) in one
step.
...
Verify: type `/fb5` in any project; the skill should appear. Spawnable agent types
`verifier` and `taste-judge` become available to the Agent tool in new sessions.

**Updating**: `/plugin update fb5@Rahul-Khare997`.
...
2. Copy the inner `fb5/` folder to `~/.claude/skills/fb5/` (the folder containing
   SKILL.md, so the final path is `~/.claude/skills/fb5/SKILL.md`).
...
- **As a custom skill**: download `fb5-skill.zip` from the repo root (or a release),
...
- **As Project instructions**: create a Project, paste the contents of
  ...
  work with most (from `fb5/references/craft/`) as Project knowledge.
- **Cowork**: point Cowork at a folder containing this repo (or grant it access), and open
  tasks with: "Read fb5/SKILL.md and the matching craft references, then execute the
  task under them."
...
2. Attach or paste the craft file matching your domain from `fb5/references/craft/`.
...
caching mechanics) live in [fb5/references/protocol.md](../fb5/references/protocol.md)
section 6 and [craft/prompting.md](../fb5/references/craft/prompting.md) section 4.

## Troubleshooting

- **`/fb5` does not appear**: skills load at session start; open a new session. For
  manual installs, confirm the path is `~/.claude/skills/fb5/SKILL.md` exactly.
...
- **The model skips phases**: invoke explicitly with a mode, e.g. `/fb5 <thing> full`,
